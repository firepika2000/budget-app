...
| Scheduled / recurring | Repeating transactions materialize on date | features | `GET/POST/PUT/DELETE /scheduled-transactions` + `POST …/realize` — **contract locked & tested** (`test_scheduled_transactions_contract`, see `docs/scheduled-transactions-api-contract.md`) | **no client create/list/realize UI** | forecast shows only | server-side (15 contract tests) | **MISSING (client)** — backend READY | **P1** | `APIClient` + scheduled-txn editor, list, and "enter now" (realize) | Create a schedule; it appears in forecast and realizes into one actual transaction |
