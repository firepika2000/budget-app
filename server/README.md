...
Purchase reserve events also identify their spending category. Refund release is capped by both the remaining payment money and that category's funded-purchase reserve, preventing a refund from consuming allocation manually set aside for old debt or reserve created by unrelated spending.
...
The script packages the database, encrypted attachment objects, integrity manifest, versioned backup
metadata (including the Alembic revision), and attachment-key recovery material, then
